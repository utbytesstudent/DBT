# Detta är en Demo för MoveNet thunder. Thunder är mer precis, men långsammare än MoveNet lightning
...
print("Input shape:", input_details[0]['shape'])  # Should print something like [1, 256, 256, 3]
...
while cap.isOpened():
    ...
    img = frame.copy()
    img = tf.image.resize_with_pad(np.expand_dims(img, axis=0), 256, 256)  # Resize to the expected size
    ...
    cv2.imshow('MoveNet Thunder - Skeleton Tracking', frame)
